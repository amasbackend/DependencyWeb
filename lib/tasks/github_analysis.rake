namespace :github_analysis do
  ...
  task :update_flags, %i[owner repo pr_number company_name] => :environment do |_t, args|
    ...
    begin
      ...
      puts "3. 確認 GitHub Access Token 已設定: ENV['GITHUB_ACCESS_TOKEN']"
  ...
  task :test_connection, %i[owner repo pr_number] => :environment do |_t, args|
    ...
    begin
      ...
      puts "\n請檢查："
      puts "1. 網路連線是否正常"
      puts "2. GitHub Access Token 是否設定正確 (ENV['GITHUB_ACCESS_TOKEN'])"
  ...
  def list_available_prs(owner, repo)
    ...
    # 使用新的認證方式
    request["Authorization"] = "Bearer #{ENV['GITHUB_ACCESS_TOKEN']}" if ENV["GITHUB_ACCESS_TOKEN"]
